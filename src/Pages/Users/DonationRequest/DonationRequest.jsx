...
const CreateDonationRequest = () => {
  const districtsRawData = useLoaderData();
  const { user } = useAuth();
  ...
  useEffect(() => {
    const found = districtsRawData.find(
      (d) => d.district === recipientDistrict
    );
    setUpazilas(found ? found.covered_area : []);
  }, [recipientDistrict, districtsRawData]);
  ...
  try {
    const res = await axiosSecure.post("/donation-request", newDonationRequest);
    if (res.status === 201) {
      Swal.fire({
        icon: "success",
        title: "Success!",
  ...
  if (status === "blocked") {
    return (
      <div className="text-center text-red-500 text-lg mt-50">
        You have been <strong>blocked</strong> by an admin and cannot create
        donation requests.
        <br />
  ...
  if (status !== "active") {
    return (
      <div className="text-center text-yellow-500 text-lg mt-10">
        You are not allowed to create a donation request.
        <br />
        Your account status is <strong>{status}</strong>. Only{" "}
        <strong>active</strong> users can proceed.
      </div>
    );
  }
  return (
        ...
        >
          ...
          <div>
            ...
            >
              ...
              {districtsRawData.map((d) => (
                <option className="text-black"
                 key={d.district} value={d.district}>
                  {d.district}
                </option>
              ))}
            </select>
          ...
          <div>
            ...
            >
              <option className="text-black" value="">Select Upazila</option>
              {upazilas.map((u) => (
          ...
          <div>
            ...
            <select
              ...
              className={selectClass}
            >
              <option className="text-black" value="">Select Blood Group</option>
              {bloodGroups.map((bg) => (
